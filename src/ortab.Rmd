...
predfunc <- function(sglt2var) {
  ...
  # mult
  ormod <- with(imprsdata, glm(formula(paste0(sglt2var, "== 'Yes' ~ ", paste(modvars, collapse = " + "))),
  ...
  for (i in seq_along(modvars)) {
    ormoduni <- with(imprsdata, glm(formula(paste0(sglt2var, "== 'Yes' ~ ", modvars[i])),
      family = binomial(link = "logit")
    ))
