...
predfunc <- function(sglt2var) {
  modvarsor <- modvars
  modvarsor[modvars == "shf_indexyearmonth_num"] <- "shf_indexyearmonth_cat"
  # mult
  ormod <- with(imprsdata, glm(formula(paste0(sglt2var, "== 'Yes' ~ ", paste(modvarsor, collapse = " + "))),
  ...
  # crude
  for (i in seq_along(modvarsor)) {
    ormoduni <- with(imprsdata, glm(formula(paste0(sglt2var, "== 'Yes' ~ ", modvarsor[i])),
      family = binomial(link = "logit")
    ))
  ...
  predall <- left_join(predall,
    ...
    mutate(Variable = case_when(
      ...
      Variable == "shf_indexyearmonth_catMarch-July 2021" ~ "March-July 2021 vs Nov 2020-Feb 2021",
      Variable == "shf_indexyearmonth_catAug-Dec 2021" ~ "Aug-Dec 2021 vs Nov 2020-Feb 2021",
      TRUE ~ Variable
    ))
