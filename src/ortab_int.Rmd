...
predfunc <- function(impdata, var, novar = var) {
...
impegfr <- miceadds::subset_datlist(imprsdata, expr_subset = !is.na(rsdata$shf_gfrckdepi_cat))
predall <- predfunc(
  impdata = impegfr,
...
```{r orforestgfradj, cache=cacheon, dependson=c("ortabintfunc", "ortabgfr", "orforestintfunc"), fig.width=5, fig.height=10, fig.cap="Adjusted associations between SGLT2i use and characteristics in GFR subgroups"}

predadj <- predall$preds %>%
  select(Variable, contains("adj"))

plotforest(forestdata = predadj, levs = levs, add1 = 3.5)
```
...
```{r orforestdiabetesadj, cache=cacheon, dependson=c("ortabintfunc", "ortabdiabetes", "orforestintfunc"), fig.width=5, fig.height=10, fig.cap="Adjusted associations between SGLT2i use and characteristics in diabetes subgroups (Type I excluded)"}
...
impdia <- miceadds::subset_datlist(imprsdata, expr_subset = !is.na(rsdata$shf_sos_prevhfh))
predall <- predfunc(
  impdata = impdia,
  var = "shf_sos_prevhfh"
)
...
```{r orforestprevhfhadj, cache=cacheon, dependson=c("ortabintfunc", "ortabprevhfh", "orforestintfunc"), fig.width=5, fig.height=10, fig.cap="Adjusted associations between SGLT2i use and characteristics in previous HFH < 1 year subgroups"}
